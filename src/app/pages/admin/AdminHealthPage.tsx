...
interface SurveyItem {
  id: string;
  title: string;
  category: '정신건강분류' | '신체건강분류';
  status: string;
}

export function AdminHealthPage() {
  const [selectedTab, setSelectedTab] = useState<'정신건강분류' | '신체건강분류'>('정신건강분류');
  ...
  const [deepSurveyCategory, setDeepSurveyCategory] = useState<'정신건강분류' | '신체건강분류'>('정신건강분류');
  const [selectedSurveyType, setSelectedSurveyType] = useState<'일일' | '정기' | null>(null);
  ...
  // 상세 페이지 상태 관리
  const [currentView, setCurrentView] = useState<'main' | 'mental-detail' | 'physical-detail' | 'risk-analysis' | 'survey-management' | 'monitoring-detail' | 'unscreened-detail'>('main');
